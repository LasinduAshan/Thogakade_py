...
__db_location__ = "db"
__session_file__ = f"{__db_location__}/login.db"
...
def order_view(id):
    order = Order()
    order.find(id)
    print("oid:",order.oid, "cid:",order.cid, "customer_name:",order.customer_name, "itmid:",order.itmid, "itm_name:",order.itm_name, "qty:",order.qty, "price:",order.price, "total:",order.total)

def order_create(itemID, customerID, qty):
...
def cust_order_create(itemID, qty):
